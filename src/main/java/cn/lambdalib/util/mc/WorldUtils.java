...
public class WorldUtils {
    ...
    public static boolean isWorldValid(World world) {
        ...
        if(SideHelper.isClient()) {
            ...
            WorldServer[] wss = ws.getMinecraftServer().worlds;
            for(World w : wss) {
                if(w == world) {
                    return true;
                }
    ...
    public static List<BlockPos> getBlocksWithin(
        ...
        fs[filter.length] = (world1, pos, block) ->
        {
            double dx = pos.getX() - x, dy = pos.getY() - y, dz = pos.getZ() - z;
            ...
            for(int y = minY; y <= maxY; ++y) {
                for(int z = minZ; z <= maxZ; ++z) {
                    ...
                    for(IBlockSelector f : filter) {
                        if(!f.accepts(world, pos, world.getBlockState(pos).getBlock())) {
                            match = false;
                            break;
                        }
                    }
    ...
    public static List<Entity> getEntities(World world, AxisAlignedBB box, Predicate predicate) {
        return world.getEntitiesInAABBexcluding(null, box, predicate);
    }
